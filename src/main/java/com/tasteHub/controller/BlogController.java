...
public class BlogController {
    ...
    @PostMapping
    @ApiOperationLog(description = "发布博客")
    public Result saveBlog(@RequestBody Blog blog) {
        return blogService.saveBlog(blog);
    }

    @PutMapping("/like/{id}")
    @ApiOperationLog(description = "点赞博客")
    public Result likeBlog(@PathVariable("id") Long id) {
        return blogService.likeBlog(id);
    }

    @GetMapping("/of/me")
    @ApiOperationLog(description = "查询我的博客")
    ...
    @GetMapping("/hot")
    @ApiOperationLog(description = "查询热门博客")
    public Result queryHotBlog(@RequestParam(value = "current", defaultValue = "1") Integer current) {
        return blogService.queryHotBlog(current);
    }

    @GetMapping("/{id}")
    @ApiOperationLog(description = "查询博客详情")
    public Result queryBlogById(@PathVariable("id") Long id) {
        return blogService.queryBlogById(id);
    }

    @GetMapping("/likes/{id}")
    @ApiOperationLog(description = "查询博客点赞数")
    public Result queryBlogLikes(@PathVariable("id") Long id) {
        return blogService.queryBlogLikes(id);
    }

    @GetMapping("/of/user")
    @ApiOperationLog(description = "查询用户博客")
    ...
    @ApiOperationLog(description = "查询关注人的博客")
